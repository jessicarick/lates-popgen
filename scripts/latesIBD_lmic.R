...
lmic_info <- left_join(data.frame(New_ID = indNames(lmic)),
                       fishinfo,by="New_ID")

lmic_info$Location[lmic_info$Location == "Kipili"] <- "Kirando"
lmic_info$Location[lmic_info$Location == "Isonga"] <- "S_Mahale"
...
Dgen_lmic <- reich.fst(lmic, bootstrap=100, plot=TRUE, verbose=TRUE)
...
lmic_locs <- locs[locs$Site %in% unique(pop(lmic)),]
...
all_dist_lmic$value.geo <- log(all_dist_lmic$value.geo / 1000)
...
## Mantel test for IBD
Dgen_lmic_fst_1fst <- as.dist(pivot_wider(all_dist_lmic[,c(1,2,5)], 
                              names_from=Var2, 
                              values_from=value.gen.std)[,-c(1)], upper=TRUE)
Dgeo_lmic_km <- as.dist(pivot_wider(all_dist_lmic[,c(1,2,4)], 
                                    names_from=Var2, 
...
vec <- read_table2("../results/emu_062021/lmic_092320_0.5_maf0.01_thin90_dp5.emu.eigenvecs",col_names=FALSE) # Reads in eigenvectors
val <- read_table2("../results/emu_062021/lmic_092320_0.5_maf0.01_thin90_dp5.emu.eigenvals",col_names="eigenval") %>%
        mutate(pct = eigenval/sum(eigenval))
fam <- read_table2("../results/emu_062021/lmic_092320_0.5_maf0.01_thin90_dp5.fam",col_names=FALSE)
colnames(fam)[1] <- "ind"

combined <- fam %>%
        dplyr::select(ind) %>%
        left_join(lmic_info,by=c("ind" = "New_ID")) %>%
        bind_cols(vec) %>%
        drop_na(all_of(c("sampling_loc","final_ID")))

lmic1 <- ggplot(data=combined, aes(x=X1,y=X2)) +
...
lmic2 <- ggplot(data=combined, aes(x=X1,y=X2)) +
...
## individual FSTs
pop(lmic) <- lmic_info$Moran_FishID
Dgen_lmic_ind <- reich.fst(lmic, bootstrap=FALSE, plot=FALSE, verbose=TRUE)
Dgen_lmic_ind_long <- melt(Dgen_lmic_ind$fsts)
