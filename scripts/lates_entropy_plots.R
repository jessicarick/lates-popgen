...
for (i in 2:6){
  ...
  data2.q <- h5read(paste("../data/admixture/",spp,"_092320_0.5_maf0.01_thin90.entropy.k",k,".rep2.hdf5",sep=""), "q")
  data3.q <- h5read(paste("../data/admixture/",spp,"_092320_0.5_maf0.01_thin90.entropy.k",k,".rep3.hdf5",sep=""), "q")
  ...
  # pdf(paste("../../lates_popgen/results/",spp,"_conv_plots_k",k,".pdf",sep=""))
  # par(mfrow=c(4,4))
...
names <- read.csv(paste0("../data/admixture/lates_all.ind"), header=T)
colnames(names) <- c("names","pop","taxon")
fishinfo <- left_join(names, details, by.x="names", by.y="names",
                      all.x=T, all.y=F)

## Extract relevant parameters from .hdf5 format
data1.q <- h5read(paste("../../../lates_popgen/data/admixture/lates_all_092320_0.5_maf0.01_thin90.entropy.k",k,".rep1.hdf5",sep=""), "q")
data2.q <- h5read(paste("../../../lates_popgen/data/admixture/lates_all_092320_0.5_maf0.01_thin90.entropy.k",k,".rep2.hdf5",sep=""), "q")
data3.q <- h5read(paste("../../../lates_popgen/data/admixture/lates_all_092320_0.5_maf0.01_thin90.entropy.k",k,".rep3.hdf5",sep=""), "q")
...
## Plot point estimates dsfor q (TO DO: add code for q/Q plot)
# pdf("q_hist_lsta_k2.pdf", width=6.5, height=9)
# par(mfrow=c(2,2))
# for(i in 1:length(unique(fishinfo$Field.ID))){
#   hist(q[which(fishinfo$Field.ID ==
